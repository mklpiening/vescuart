...
const unsigned short crcTable[] = { 0x0000, 0x1021, 0x2042, 0x3063, 0x4084,
...
void VescMessage::addCRC() {
    unsigned int i;
    unsigned short cksum = 0;
    for (i = 0; i < size(); i++) {
        cksum = crcTable[(((cksum >> 8) ^ at(i)) & 0xFF)] ^ (cksum << 8);
    }
...
VescMessage VescMessage::finalize() const {
    VescMessage finalMessage(*this); 

    // add crc
    finalMessage.addCRC();
    
    // add first two bytes: 2 <length>
    finalMessage.insert(finalMessage.begin(), size());
    finalMessage.insert(finalMessage.begin(), 2);
    
    // add termination byte
    finalMessage.add<uint8_t>(3);
